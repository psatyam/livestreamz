...
class User_model extends CI_Model {
    ...
    function registerUser($formdata) {
        ...
        $this->load->dbforge();
        $posts_fields = array(
            'int_gallery_id' => array('type' => 'INT', 'constraint' => 9, 'unsigned' => TRUE, 'auto_increment' => TRUE),
            'int_gallery_url' => array('type' => 'INT'),
        );
        $table_name = 'tab_gallery_' . $lastInsertId;
        $this->dbforge->add_field($posts_fields);
        $this->dbforge->add_key('int_gallery_id', TRUE);
